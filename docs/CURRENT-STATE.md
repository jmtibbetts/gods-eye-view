...
| Conjunctions 💥 | CelesTrak SOCRATES (`sort-minRange.csv`) + per-object GP elements | `src/layers/conjunctions/` via `src/app/layers/conjunctions.js` | `/api/space/conjunctions` (cuts 150k rows to ≤100; elements fetched one at a time in the background, disk-cached; `pending` counts the rest) | 15 min (45 s while elements pend) |
...
- **Satellites**: ~1,040-sat core catalog (stations/weather/resource/visual/GPS/GLONASS/Galileo/GEO — weather and resource are the Earth-observation imagers, tagged EARTH OBS, and are loaded ahead of visual and geo so first-tag-wins classes them by what they do), a SENSORS panel (`src/ui/sensorsPanel.js`) that names the tracked imager's instruments from `src/layers/satellites/sensors.js` and switches imagery products through the IMAGERY panel's own path, a swath-strip / disk footprint under a tracked imager (`src/layers/satellites/footprint.js`, rebuilt on the one-second beat, ground-classified onto terrain and 3D tiles), pass prediction for any catalogued satellite (`src/data/satellitePass.js`: `findNextPass`, the ISS finder generalised, and `findNextImagingPass`, the swath's closest approach to a point with off-track distance and a solar-elevation daylight check; surfaced in SENSORS as PASSES OVER the pre-tracking view centre, or the device's position on request), a frame loop for rolling geostationary products (`src/layers/imageryOverlays/frames.js`: GIBS DescribeDomains + one probe per candidate frame, twelve frames as stacked imagery layers with one alpha at a time, offered from SENSORS as PLAY THE LAST 2 H), tracking lands ~726km out via `viewFrom` (tracked entity owns a point graphic so the tracking camera engages), rings realign via primitive modelMatrix (no per-second rebuild flicker), optional `setParams({catalog:'dense'})` Starlink mode.
- **Satellite classes** (`src/data/satelliteClass.js`): every satellite is classified from the CelesTrak group it was ingested with — no extra fetch and no heuristics — into STATION (warm white), EARTH OBS (green; the weather and resource groups — the imaging fleet, sub-labelled WEATHER or IMAGING), NAV (cyan; GPS/GLONASS/Galileo deliberately share one color so the GNSS family reads as one thing), GEO (violet), VISUAL (muted blue-gray catch-all), and COMMS (dim slate, dense Starlink shell only). That module is the single source of truth for class, label, and color, so the dot, the card, and the legend swatch cannot disagree. Two deliberate palette rules: no class may sit in the 40–48° amber band, which is the app-wide known-military convention; and COMMS stays far below VISUAL in Rec.601 luminance so the dense shell stays separable when NVG/FLIR collapse the scene to one channel. The ISS keeps its long-standing red hero dot rather than the STATION color — it carries a permanent name label and its card still reads `STATION · ISS`. `satelliteClassOf` is the single owner of that ISS rule, so the card label and the legend tally can never disagree: during a stations-feed outage the ISS is ingested as `visual`, and both surfaces still file it under STATION.
...
- Radiofax is a station catalog with schedules. `src/layers/sdr/weatherfax.js` holds twelve HF fax stations (NMF, NMG, NMC, NOJ, KVM70, GYA, DDH/DDK, JMH, JFX, VMC, VMW, RBW) with approximate transmitter positions and published frequencies from the NWS Worldwide Marine Radiofacsimile Broadcast Schedules. `wefaxDialHz` applies `WEFAX_USB_OFFSET_HZ` (−1900 Hz) because radiofax is received as SSB and the published number is the carrier — tuning it directly yields noise. Frequencies carry a `window` of UTC minutes or null for continuous, and `withinWindow` handles a window whose start exceeds its end (Honolulu's 16135 kHz runs 1719–0356Z). `chooseWefax` takes the nearest station, orders its currently-active frequencies by closeness to 8 MHz — the least bad single guess, since nothing here models propagation — and returns the first a receiver covers, with the rest as `alternatives`. The `wefax` marine band is a placeholder frequency flagged `wefax: true`; `_resolveListenTune` in `src/ui/audioPanels.js` resolves it at LISTEN time from the view anchor, the same hook the airband preset uses to resolve to the nearest tower. A test asserts every station has something on air at every hour.
- Marine VHF that no receiver can hear is handed to Broadcastify. `marineFeedListingFor` (`src/ui/audioPanels.js`) returns the US state listing when the region is known and the directory front page otherwise. It fires in two places: the Receivers panel when the `vhf-ch16` preset finds nothing within `VHF_MAX_KM`, and `InspectPanel._listen` when `listenMarineNear` refuses a ship outright. `InspectPanel._marineFeedListing` resolves the region from the nearest airport in the bundled ATC directory, so it needs no network call. This mirrors the existing LiveATC hand-off: the app says plainly that it cannot play the audio and points at who can, rather than failing silently. Broadcastify serves 403 to scripted requests, so there is deliberately no bundled feed pack — the app links the listing rather than claiming feed ids it cannot verify.
- The marine bands are one catalog. `src/layers/sdr/marine.js` holds ten `MARINE_BANDS` — VHF Channel 16, MF 2182 kHz, the five HF radiotelephony distress/calling frequencies (4125, 6215, 8291, 12290, 16420 kHz), NAVTEX 518 kHz and DSC 2187.5/8414.5 kHz — each with `kind` (`voice` or `data`), `maxKm`, and `ladder`, its position in the ship LISTEN order or null. `MARINE_LADDER` is the voice-only, ordered subset `chooseMarineListen` walks; a test asserts no `data` band can reach it, because a LISTEN that opens a data carrier is not listening to a ship. `marinePresets()` shapes three of them (`MARINE_PRESET_IDS`: Channel 16, 2182 kHz, NAVTEX) into the Receivers panel's preset form, and `SDR_PRESETS` in `src/ui/audioPanels.js` spreads that in rather than restating the frequencies — the panel buttons and the ship's ladder cannot drift. The exact GMDSS frequencies are pinned in `marine.test.mjs` so a typo in a distress frequency fails there rather than sending someone to an empty channel. Preset buttons live in `src/ui/templates/context.html` keyed by band id (`vhf-ch16`, `hf-2182`, `navtex-518`).
- A selected vessel can be listened to. `inspectActions` gives `ais-live-vessels` a LISTEN action when the SDR layer is present and the record carries a position, and `InspectPanel._listen` calls `listenMarineNear({lat, lon})` on it, enabling Receivers first if needed. The band ladder is `src/layers/sdr/marine.js` (pure, node-testable): Channel 16 (156.800 MHz NBFM) first with a 120 km ceiling, then 2182, 8291, 6215, 4125, 12290 and 16420 kHz USB with no ceiling. `chooseMarineListen` takes a `receiversFor` callback so the directory stays with the layer, walks the ladder, and returns the first band whose nearest covering receiver is inside that band's reach. The ceiling exists because above `VHF_FLOOR_HZ` (30 MHz) radio is line-of-sight and only 39 of the 1,201 directory receivers cover 156.8 MHz, against 731–774 for each marine HF frequency; HF is ionospheric, so its ceiling is Infinity by design rather than by omission. (Counted with the layer's own `coveredOnly` rule, which admits a receiver only when its published bands say yes — a further 354 rows publish no bands at all and are never counted as covering anything.) A refusal reports the nearest Channel-16 receiver's distance when one exists (`marineRefusalText`), because that distance is the useful fact — it says the ship is out at sea rather than that the directory is empty.
- Place names are drawn by the world overlay, not by Cesium labels. `src/layers/placeNames/` names six tiers — continents, countries, oceans, seas, mountain ranges and deserts, and cities — on whichever basemap is active, which matters because Google 3D, Esri Satellite and Bing Aerial carry no labels of their own. The layer owns no scene objects: `refresh()` builds a cohort on a `PLACE_NAMES_VIEW_TICK_MS` (250 ms) tick gated on `cameraPoseSignature`, and hands it to `overlayHost.setEntries`, which already solves projection, collision against every other ambient label, and horizon culling (`horizonCull: true`, `terrainOcclusion: false` — under photoreal there is no globe depth to test). Tier visibility is decided by camera height in `policy.js` (`tiersForHeight`), and the city tier is additionally filtered by `cityRankCeiling`, a step function over Natural Earth's own `scalerank`; the steps are coarse on purpose, because a continuous threshold adds and drops single towns as the camera drifts and reads as flicker. A national capital gets `CAPITAL_RANK_BONUS` so it is labelled before larger non-capitals.
- Label anchors are poles of inaccessibility, precomputed. `src/data/placeAnchors.js` finds the point inside a shape furthest from any edge, because a centroid is outside a crescent — Norway's is in Sweden, Chile's in Argentina. The search costs ~1 ms per shape and there are 1,083 shapes, so it runs in `scripts/curate-place-label-anchors.mjs` and the result is bundled as `natural_earth_places/anchors.json` (84 KB); the runtime build is then 23 ms. `buildPolygonAnchors` is the build-time entry point, `buildPlaces` the runtime one.
- A country is named while you are inside it. A country anchored at one point loses its label as soon as that point leaves the screen, which inverts the intent — the closer you are, the more you want to be told where you are. `visiblePlaces` therefore also names the country containing the view centre, pinned to that centre when its own anchor is off screen, deduplicated against the anchor-placed entry. Containment is `countryAt`, a point-in-polygon test over the bundled 167 KB country outlines rather than a bounding box, because a box names France while the camera is over Belgium. The other polygon tiers stay anchor-placed: over open ocean with no anchor in view the layer names nothing rather than guessing.
- Bundled JSON packs are imported plainly first. Node requires the `type: 'json'` import attribute; Vite rewrites a module's JSON import to `…?import` and serves JavaScript, which then fails that attribute — so every pack loaded under `node:test` and none loaded in the browser, including the region and marine packs behind the voice resolver's "outline the Alps". `importJsonPack` (`src/data/naturalEarthRegions.js`) takes two thunks with literal specifiers, tries the plain one, and falls back to the attributed one for Node.
- `place-names` is the only layer that ships enabled. `createDefaultLayerState()` lists it, which is safe because both a share link and a stored session state their enabled set in full — `l=` is always emitted, and `l=` empty is the valid explicit-empty form — so a non-empty default cannot change what an existing link or saved session means. An existing user whose stored state predates the layer therefore starts with it off, and switching it on once persists as usual.
- Conjunction markers are horizon-culled. Their points carry `disableDepthTestDistance: Number.POSITIVE_INFINITY` so a marker at orbital altitude is not swallowed by the surface below it; that also exempts them from the far side of the Earth, which under photoreal writes no depth at all. `cullToHorizon` in `src/layers/conjunctions/index.js` sets each entity's `show` from `horizonOccluder(camera).isPointVisible(...)` (`src/data/iconOrientation.js`, the same occluder the flights, vessels, CCTV, FIRMS and radio layers use), gated on `cameraPoseSignature` so a still camera costs one string compare. It runs on a `CONJUNCTIONS_VIEW_TICK_MS` (250 ms) tick from its own `viewTimers` seam — separate from the one-shot `timers` seam that schedules pending-element retries — and is forced at the end of `rebuild()`, because a replaced entity defaults to shown and would otherwise wait for a camera move to be judged. The ellipsoid horizon is the right boundary here rather than a ground-level one: a marker high enough to clear the limb genuinely is visible.
- Every other always-on-top point layer already culls. The marker-field layers (`atc`, `sdr`, `scanner`, `lightning`, `river-flood`, `volcanoes`, `storm-reports`, `tropical`, `satnogs`) reach zero markers-through-the-Earth on their own once their cull has run; counts taken while a layer is still populating are a load-time transient, not a leak. Audited live from 3 km to 14,000 km after the conjunctions fix: zero entities and zero always-on-top primitives drawn through the planet.
- Imagery is draped onto the photoreal surface rather than displacing it. Cesium's `ImageryLayer` paints on the globe, which `globe.show = false` removes under Google 3D, so an overlay added there requested no tiles and drew nothing. The conclusion drawn from that — that imagery cannot be shown over Google 3D — held only for that renderer: a `rectangle` whose material is an `ImageMaterialProperty` and whose `classificationType` is `CESIUM_3D_TILE` is draped onto the rendered tiles, following their geometry, exactly as the weather-alert polygons already are. `src/layers/imageryOverlays/drape.js` owns that path: one WMS `GetMap` image for the camera's padded view rectangle, re-requested on `camera.moveEnd`, decoded before it is swapped in so the surface is never left bare mid-pan, with a request token so a slow frame cannot paint over a newer one. `drapeSourceFor` (catalog) gives every product its single-image form — GIBS layers through the GIBS EPSG:4326 WMS, the rest through their own service — which is what makes draping a rule rather than a favour to particular products; GIBS's newest frame is asked for by sending no `TIME`, since `default` is a REST path segment it cannot parse as a date. `createImageryOverlayLayer` chooses the renderer from `globe.show` at the moment of drawing, not from a remembered stack, and re-chooses on `gev:map-stack-changed`, so leaving the 3D surface hands the overlay back to the imagery layer and arriving at it takes the inert layer down. A draped overlay never retains the surface coordinator, and `_retained` makes disable release exactly what enable took — releasing a hold it never had would decrement another overlay's and leave the last real holder unable to restore the basemap.
- Radar keeps the borrow. `imagery-radar` resolves a tile template from RainViewer, which serves no WMS, so `drapeSourceFor` returns null for it and it still switches to a globe stack it can be seen on. The IMAGERY panel's note no longer claims imagery cannot draw over Google 3D; the basemap-swap sentence in the sensor toast survives for the case that still swaps and names the product rather than the whole class.
- A tileset that stops loading is renewed before it is given up. Google's photorealistic tiles carry a session token issued with the root request and expired by Google after a while; once it lapses every content request answers `400 INVALID_ARGUMENT`, Cesium marks each tile failed and stops retrying, and the surface draws nothing — which, with `globe.show = false` beneath it, is empty space rather than a plainer map, and leaves far-side satellites with nothing to occlude them. `MapController._watchTileset` counts `tileFailed` events and after three calls the source's `renewTileset` (wired in `src/app/scene.js` to `loadPhotorealisticTileset`), adds the fresh tileset, shows it, disposes the old one, and watches the replacement. Renewal happens at most once per activation (`_renewedTilesetGen`): a renewed surface that fails again falls back to `tileFailureFallback` — Bing Aerial with an ion token, Esri without — because a surface that cannot draw twice is not a stale session. Leaving the stack releases the listener, so a late failure cannot move the map out from under a later choice.
- Fallbacks nobody asked for are announced. `createMapSourceControls` reports `lastError` once for every state it observes, not only for states that answered a click, so an autonomous stack change raises the same toast a rejected click does. `setStack()` clears `lastError` at the start of each switch, so the same failure recurring is announced again rather than swallowed as a duplicate.
...
## Road-fetch geometry, Overpass patience and receiver identity

The street-traffic fetch box is square on the ground rather than square in
degrees. `clampBoundsAroundCenter` treats its `maxSpanDeg` as a latitude span
and divides the longitude cap by cos(latitude), floored so the span stays
finite near the poles. Capping both axes at the same number of degrees made
the box 5.57 km tall but 4.22 km wide in New York, 3.46 km in London and
2.79 km in Oslo, and because the viewport is wider than it is tall the missing
coverage fell on the axis that needed it most. Query area is now the area the
0.05° cap was chosen to buy, at every latitude. The box remains a fixed ground
size, so at the top of the activation band it still covers part of the view.

The Overpass proxy spends one deadline across the whole mirror fan-out instead
of giving each mirror the full transport timeout. `sanitizeOverpassBody`
reports the clamped `[timeout:]` the query declared and `overpassBudgetMs`
turns it into a budget — the declared seconds plus a fixed transfer allowance,
never more than the old mirrors × timeout worst case. Before each further
mirror the transport checks what is left and stops rather than opening a
window too small to answer in; the first mirror always gets its turn. A layer
that asked for twelve seconds of Overpass work no longer waits eighty-eight
for four mirrors in turn, and a healthy first mirror behaves exactly as before.

The bundled receiver directory is folded to one entry per radio.
`foldSdrDuplicates` merges rows that share a name, a position and a port —
a dynamic-DNS name, the kiwisdr.com proxy that reaches the same box from
behind a carrier NAT, and its raw IP — keeping the losing addresses on the
survivor as `alternateUrls`. https wins the primary address over http and a
hostname wins over a bare IP; a different port on one host stays a separate
entry, because that is a second receiver rather than a second way into the
first. A `placeMismatchKm` warning carried by any folded row survives onto the
survivor, so a cleanup cannot turn an unverified pin into a silent one. The
1,317-row snapshot presents as 1,201 receivers with 116 alternate addresses.
