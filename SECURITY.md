...
- **Scanners and SDR receivers have no proxy by design.** The scanner layer calls OpenMHz from the browser (CORS-open, keyless; its edge challenges non-browser clients) and plays recorded transmissions through a plain audio element — only `https://media2.openmhz.com/*.m4a|mp3|wav` URLs with no query or credentials are ever assigned to it, so a malformed catalog row cannot point the element elsewhere. The SDR layer never connects to a receiver: it opens the receiver's own page in a new tab with `noopener`, and receiver URLs are limited to public `http(s)` hosts (no credentials, no loopback/private ranges).
