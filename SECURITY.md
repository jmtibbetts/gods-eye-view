...
- **Scanners and SDR receivers have no proxy by design.** The scanner layer calls OpenMHz from the browser (CORS-open, keyless; its edge challenges non-browser clients) and plays recorded transmissions through a plain audio element — only `https://media2.openmhz.com/*.m4a|mp3|wav` URLs with no query or credentials are ever assigned to it, so a malformed catalog row cannot point the element elsewhere. The SDR and ATC layers never connect to a receiver or to LiveATC: they show the receiver's own page (or LiveATC's airport page) inside the in-map receiver window — a plain, unsandboxed cross-origin iframe with `referrerpolicy="no-referrer"` and only `allow="autoplay"`, so the framed page gets no more than any tab would and nothing from GEV — or in a new tab with `noopener`. URLs handed to the window are limited to public `http(s)` hosts (no credentials, no loopback/private ranges). The ATC layer bundles frequencies only; LiveATC's terms forbid third-party use of its audio streams, so no stream URL is ever assigned to an audio element.
