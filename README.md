...
| ----- | -------------- |
| 🌍 **The planet stays drawn** | Google's 3D tiles are served against a session token the tileset picks up with its first root request, and Google expires it. Nothing renewed it, so after a while every tile answered `400` and the surface drew nothing — and because that stack hides the globe underneath it, "nothing" was empty space where the Earth had been. The stack now watches its own tiles and fetches a fresh root, and a fresh session, after three failures. Once per activation: a surface that fails again after renewal cannot draw at all, so it falls back to the globe rather than renewing forever |
| 🛰️ **Imagery draws ON Google 3D** | Turning on any sensor used to switch your basemap to the 2D globe, because a Cesium imagery layer paints on the globe and the globe is hidden under the tiles. True of a *layer*; not true of an *image*. A rectangle classified onto the rendered tiles drapes on the 3D surface and follows its geometry — over a city it wraps the buildings. Every product in the catalog is also a WMS layer, so each one can be asked for as a single picture of whatever the camera is looking at, re-requested when the camera settles. Leaving the 3D surface hands the overlay back to the imagery layer, which is sharper where there is a globe to paint on. Radar is the exception and says so: it comes from a tile service that speaks no WMS |
| 💥 **Nothing draws through the Earth** | A conjunction marker is drawn at its own altitude with the depth test off, so a marker a few hundred kilometres up is not swallowed by the ground it is over. Nothing then stopped one on the *far* side of the planet being painted over the near side, and its stalk to the ground *is* depth-tested — so they arrived as bare red dots with nothing underneath, drifting as the camera moved. They cull to the horizon now, on the same shared occluder the flights, CCTV and radio layers already use — the same pass that floored and horizon-culled the static point layers, which had been drifting against the terrain as well as showing through it. Audited live from 3 km to 14,000 km: nothing drawn through the planet |
| 🗺️ **A map that changes itself says so** | A basemap fallback was announced only when it followed a click, so a source that failed on its own moved the map under you with nothing but an amber MAP label to show for it. Every state the tray sees now reports its error once, whoever caused it — and a renewal that quietly works stays quiet |
| 🔗 **Share links that survive their own length** | A link with every layer on did not decode — and it was rejected *whole*, so the recipient got default state with nothing to say anything had been dropped, which reads as the sender having had nothing on. The `l=` ceiling was 64 characters, written when sixteen one-character tokens was a generous allowance; enough layers later a maximal link encoded past it, and the guard had quietly crossed from rejecting hostile input to rejecting honest input. The ceiling is derived from the layer registry now, so adding a layer cannot bring it back, and it still fails closed on junk |
| 🩺 **A feed that fails says so** | A layer whose load failed used to look the same as one with nothing to report. Failures surface on the row, a flaky upstream is survived rather than fatal, and the POWER UP panel lists every source that needs no key at all — so what is missing is legible as missing, not as empty |
| 🔤 **Rows and cards that read straight** | A pass over what the panels actually say. A layer that has never been switched on names its source instead of reporting "never" like a broken feed; a selected thing is credited to its source once rather than three times over; an ISO timestamp is rendered as a time a reader asked for rather than the string the feed sent; and the dock names the window it is, whether that is a waterfall, a webcast or a video. The reasoning is in [docs/UX-REVIEW.md](docs/UX-REVIEW.md) |
...
- **🚀 LAUNCH — watch it go, hear the range:** The next dozen launches from Launch Library 2 as live countdowns, in flight, on hold or flown, with the operator's latest note under each. WATCH opens the webcast in the receiver dock when the publisher allows framing and on their own page when they do not; GO TO PAD puts the camera on the pad with the clock hanging over it, so the count, the pad and the stream are on screen together. RADIO is the honest part. The countdown net and the flight loops you hear on a webcast are the operator's own circuits, mixed into their stream — no public receiver hears them, and this panel never pretends one does. What a radio near the range CAN hear, it names: the Kennedy Space Center trunked system that OpenMHz carries, a clip per transmission; the tower and approach frequencies of the strips on the range and the airfields under the closure, which LiveATC opens in its own tab; and the web receivers nearby, with every HF-only KiwiSDR marked as unable to hear a launch rather than offered as a way in. RANGE is what is published around the pad: the FAA's space-operations closures within 250 km with the NOTAM's own start, end and altitude — a closure opens hours before the window and is often the firmest public sign of when the operator means to fly — and SHOW turns on Flight Restrictions and flies to it; and the droneship the booster is coming back to, named when Launch Library names it and otherwise the ships that work that coast, one PIN from the WATCHLIST with Live Vessels switched on — pinned by MMSI, because the barges broadcast their hull names (MARMAC 304, not "Of Course I Still Love You"). ALERT T−10 is a browser notification ten minutes before net, for the launch you would otherwise miss with the tab in the background; it asks for permission once and falls back to a toast when refused.
...
_Each sensor says what it is, how far behind live it runs, and how far back its archive goes, so nothing here is presented as more current than it is. A sensor is painted onto whichever map is showing, Google 3D included, so turning one on no longer costs you the photorealistic basemap. CLEAR takes it off again._
