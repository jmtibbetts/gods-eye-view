...
🛠️ **This fork adds** live police / fire / EMS scanners · web SDR receivers · ATC frequencies · NWS weather alerts · SPC storm reports · USGS volcano alerts · 24 switchable NASA satellite sensors · region monitors · contact watchlists · sanctioned-and-dark ship detection · an imagery timeline · snapshot export — **[see what's new →](#-what-this-fork-adds)**
...
| 🛰️ **IMAGERY — 24 sensors** | Switch the globe between 24 NASA GIBS products grouped by what they reveal: true colour from four platforms, false colour for fire and for snow-vs-cloud, the day/night band, GOES-East and GOES-West, Himawari over the Pacific, clean infrared, dust, fire temperature, air mass, plus sea-surface temperature, aerosol, snow cover and sea ice. Each one states its own publication lag |
...
- **🛰️ IMAGERY — 24 sensors, one globe:** The same planet through twenty-four different instruments, grouped by what each one actually reveals. True colour from four platforms at four overpass times. False colour where shortwave infrared separates burning fire from old burn scar, and ice from the cloud sitting on top of it. The day/night band, which sees last night by its own light — cities, gas flares, fishing fleets. The geostationary pair plus Himawari, so the Pacific is not a blind spot. Dust, fire temperature and air mass. And measured science on a colour ramp: sea-surface temperature, aerosol, snow cover, sea ice. Every sensor states the lag it is actually running at, because a ten-minute geostationary frame and a three-day snow-cover composite are not the same claim, and presenting them identically as "satellite imagery" would be the one lie this feature could tell.
...
Twenty-three layers and map sources. **Twenty-one have a keyless path.** Some offer additional capabilities with a provider key. (🟢 no key · 🟡 free key · 🔴 metered.)
...
| 🛰️ **IMAGERY (24 sensors)** | Three switchable GIBS slots plus radar, stackable under everything else. ORBITAL: true colour from NOAA-20, Suomi-NPP, Terra and Aqua at four different overpass times; false colour for fire and burn scars (M11-I2-I1) and for ice against cloud (M3-I3-M11); the VIIRS day/night band; the Black Marble composite. GEOSTATIONARY: GOES-East and GOES-West GeoColor, Himawari over the Pacific, clean longwave infrared on all three, plus dust, fire temperature and air mass RGBs. MEASURED SCIENCE: sea-surface temperature and its anomaly, aerosol optical depth, snow cover, land-surface temperature, sea ice. Each product carries its verified publication lag and archive depth — Terra reaches back to February 2000 — and `npm run check:imagery` re-probes all 24 so a retired product fails loudly instead of serving a silent black tile | RainViewer + NASA GIBS | 🟢 |
