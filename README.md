...
| 🛰️ **SENSORS** | Track NOAA-20, Terra, a Sentinel, GOES or Himawari and the panel names the instrument it carries — swath, resolution, bands — and offers every product the globe can draw from it as a button, while the satellites layer paints the ground the sensor is sweeping under it as it moves. Every picture is labelled with how far behind live it is published, because nothing outside an operator's control room commands a spacecraft |
